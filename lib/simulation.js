...
import { RMD_PARAMETERS } from './tax-parameters.js';
 ...
 * Simplified progressive tax estimate (2026 brackets, single filer)
...
export function calculateTaxRate(withdrawal, ssIncome) {
  ...
  if (totalIncome <= 12400) return 0.1;
  if (totalIncome <= 50400) return 0.12;
  if (totalIncome <= 105700) return 0.22;
  if (totalIncome <= 201775) return 0.24;
  if (totalIncome <= 256225) return 0.32;
  if (totalIncome <= 640600) return 0.35;
  return 0.37;
}
 ...
 * Re-exported from tax-parameters for convenience
 */
export const RMD_START_AGE = RMD_PARAMETERS.startAge;
...
export function calculateRMD(balances, age) {
  ...
  const period = RMD_PARAMETERS.uniformLifetimeTable[lookupAge] || 2.0;
